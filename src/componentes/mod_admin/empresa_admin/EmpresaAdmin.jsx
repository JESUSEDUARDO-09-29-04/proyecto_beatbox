...
const EmpresaAdmin = () => {
  ...
  const sanitizedValue = String(nuevoValor)
    .replace(/<\/?[^>]+(>|$)/gi, "")
    .trim()
  ...
  const actualizarCampo = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (seccion === "perfil") {
          setPerfil((prev) => prev.map(([key, valor]) => (key === campo ? [key, sanitizedValue] : [key, valor])))
        } else {
          setConfiguracion((prev) => prev.map(([key, valor]) => (key === campo ? [key, sanitizedValue] : [key, valor])))
        }
                ...
                <tbody>
                  {perfil.map(([campo, valor]) => (
                    <tr key={campo}>
                      ...
                      <td>
                        <button className="btn-actualizar" onClick={() => abrirModal(campo, valor, "perfil")}>
                          Modificar
                        </button>
                      </td>
                    </tr>
                ...
                <tbody>
                  {configuracion.map(([campo, valor]) => (
                    <tr key={campo}>
                      ...
                      <td>
                        <button className="btn-actualizar" onClick={() => abrirModal(campo, valor, "configuracion")}>
                          Modificar
                        </button>
                      </td>
                    </tr>
                ...
                {logos.map((logo) => (
                  <div key={logo._id} className="logo-item">
                    ...
                    <div className="logo-buttons">
                      <button className="btn-vigente" onClick={() => setLogoAsVigente(logo._id)}>
                        Hacer Vigente
                      </button>
                      <button className="btn-eliminar" onClick={() => deleteLogo(logo._id)}>
                        Eliminar
                      </button>
                    </div>
                  </div>
